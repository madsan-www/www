...
exports.onCreateNode = ({ node, node: file, actions }) => {
  ...
  if (node.internal.type === `MarkdownRemark`) {
    ...
    const slug =
      file.frontmatter.path || slugFileName.length >= 10
...
exports.createPages = async ({ graphql, actions }) => {
  ...
  contentMarkdown.forEach(({ node: file }) => {
    ...
    const slug = file.frontmatter.path || file.fields.slug;
    ...
    const template = isPage
      ? path.resolve(`src/templates/${String(isPage)}.js`)
      : postTemplate;
